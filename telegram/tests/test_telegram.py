...
def test_the_prompt_names_where_the_decoder_actually_is(tmp_path):
    """The prompt tells the agent to run the decoder through shell, and the
    decoder moves with the plugin, so the path cannot be a literal in the file.
    """
    source = tmp_path / "prompt.txt"
    source.write_text("run shell {DECODER} path\n", encoding="utf-8")

    rendered = tm._render_prompt(str(source))
    text = open(rendered, encoding="utf-8").read()

    assert "{DECODER}" not in text
    assert tm._plugin_file(tm.QR_DECODER) in text
    assert os.path.isfile(tm._plugin_file(tm.QR_DECODER)), "decoder is not shipped"


def test_a_prompt_without_the_placeholder_is_left_alone(tmp_path):
    """An operator pointing TG_PROMPT_PATH at their own file gets that file,
    not a rewritten copy of it."""
    source = tmp_path / "prompt.txt"
    source.write_text("no placeholder here\n", encoding="utf-8")

    assert tm._render_prompt(str(source)) == str(source)


def test_an_unreadable_prompt_is_reported_and_handed_back(tmp_path):
    """Rendering must not turn a missing prompt section into a crash on load:
    the MeTTa side already warns and skips when the path does not exist."""
    missing = str(tmp_path / "gone.txt")
    assert tm._render_prompt(missing) == missing


def test_the_shipped_prompt_carries_the_decode_instructions():
    text = open(tm._plugin_file("prompt.txt"), encoding="utf-8").read()
    assert "{DECODER}" in text
    assert "[IMAGE FILE: path]" in text
    assert "NO_QR_CODE_FOUND" in text
