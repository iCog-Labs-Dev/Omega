...
For an Omega agent, MeTTa is not merely a convenience language. It is the medium in which symbolic control, orchestration, reflective rewriting, and cross-component coordination become uniform. In practice, Omega should be read as inheriting MeTTa’s role as shared cognitive calculus, with high-level agent logic remaining MeTTa-facing even when lower-level performance paths are delegated to PeTTa/MeTTaTron, MM2, MORK, or ASI:Chain-aligned execution routes.
...
- Omega-specific wording is future-facing representing our intentions for the Omega roadmap.
