...
a commit whose subject contains the run_id вЂ” all via shell calls. The
...
from helpers import (
    Checker, dexec, dexec_root, find_skill_calls, make_prompt, setup_git_in_container, teardown_git_in_container, wait_for_file,
)
...
def test_git_local_commit_slack_mock(llm, sl):
    with Checker("git local commit (slack mock)", cleanup_dirs=[TARGET_DIR]) as c:
        ...
        c.step("send prompt via Slack with mocked response")
        prompt = make_prompt(
            c.run_id,
        ...
        # add, commit. Quote handling is tricky here вЂ” git commit -m
